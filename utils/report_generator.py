...
def make_pdf(candidate: dict, scores: dict, results: list[dict], feedback: dict) -> bytes:
    ...
    summary = [["Overall", f"{scores['overall']}/100"], ["Technical / answer", f"{scores['technical']}/100"], ["Communication", f"{scores['communication']}/100"], ["Voice feature indicator", f"{scores['voice']}/100"], ["Expression-based indicator", f"{scores['behavior']}/100"], ["Eye-visibility indicator", f"{scores.get('eye_contact', 0)}/100"], ["Confidence-related indicator", f"{scores.get('confidence_indicator', 0)}/100"]]
    ...
    for i, result in enumerate(results, 1):
        ...
        story.append(Paragraph(f"Answer: {result['answer_score']:.0f}/100 | Voice indicator: {result.get('voice',{}).get('voice_score',0):.0f}/100 | Eye visibility: {result.get('eye_contact',{}).get('label','Not available')} | Detected facial expression: {result.get('emotion',{}).get('dominant_emotion','Not available')}", styles["BodyText"]))
