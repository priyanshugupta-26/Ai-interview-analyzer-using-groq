...
- One-question-at-a-time live browser camera/microphone workflow through WebRTC, with automatic Groq transcription and a typed transcript fallback
...
Voice analysis computes RMS energy, silence ratio, duration, and level consistency from browser-recorded audio. DeepFace, when installed and operational, samples a camera frame and reports a detected facial expression. OpenCV also reports whether eyes are visible in the frame; this is an eye-visibility/camera-engagement proxy, not proof of eye contact. These values are supportive, non-diagnostic indicators—not proof of confidence, nervousness, personality, emotion, or suitability.
...
# Ai-interview-analyzer-using-groq
